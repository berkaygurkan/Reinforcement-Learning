...
# %% [2] CONFIGURATION (FRICTION FOCUSED)
@dataclasses.dataclass
class ExperimentConfig:
    experiment_name: str = "MetaReacher_Friction"
    env_id: str = "Reacher-v4"
    seed: int = 42
    
    # EĞİTİM (Baştan eğitim şart çünkü sürtünmeyi öğrenmeli)
    total_timesteps: int = 3_000_000  # 3M adım yeterli olabilir
    n_envs: int = 8 
    
    # TEST SENARYOSU
    change_step: int = 10           # 50. adımda arıza oluşsun
    friction_multiplier: float = 6.0 # Sürtünme 6 katına çıksın (Çok sert bir fren etkisi)
    
    result_dir: str = "./results_friction"

CFG = ExperimentConfig()
seed_everything(CFG.seed)
os.makedirs(CFG.result_dir, exist_ok=True)

# %% [3] FRICTION META-WRAPPER
class FrictionReacherWrapper(gym.Wrapper):
    def __init__(self, env, is_training: bool = True, dr_mode: bool = False, 
                 dynamic_test_mode: bool = False,
                 change_step: int = 10,
                 target_friction_scale: float = 1.0):
        ...
        self.target_friction_scale = target_friction_scale
        ...
        if hasattr(self.unwrapped, "model"):
            # Orijinal sürtünme değerlerini sakla
    ...
    def reset(self, **kwargs):
        ...
        # Resetleyince orijinale dön
        if self.original_damping is not None:
            self.unwrapped.model.dof_damping[:] = self.original_damping[:]
        
        # Eğitimde rastgele sürtünme (0.5x ile 2.0x arası)
        if self.is_training and self.dr_mode:
            scale = np.random.uniform(0.5, 2.0)
            self._apply_friction_scale(scale)
    ...
    def step(self, action):
        ...
        # --- TEST SIRASINDA SÜRTÜNME ARTIRIMI ---
        if self.dynamic_test_mode:
            if self.step_count == self.change_step and not self.friction_changed:
                if self.target_friction_scale != 1.0:
                    print(f"[DEBUG] Step {self.step_count}: Sürtünme {self.target_friction_scale}x artırılıyor!")
                    self._apply_friction_scale(self.target_friction_scale)
    ...
    def _apply_friction_scale(self, scale: float):
        if self.original_damping is not None:
            new_damping = self.original_damping.copy()
            new_damping[:] *= scale 
            self.unwrapped.model.dof_damping[:] = new_damping

# %% [4] FACTORY
def make_env(rank: int, is_training=True, dr_mode=False, dynamic_test_mode=False, 
             change_step=50, target_friction_scale=1.0, log_dir=None):
    def _init():
        env = gym.make(CFG.env_id)
        env = FrictionReacherWrapper(
            env, 
            is_training=is_training, 
            dr_mode=dr_mode, 
            dynamic_test_mode=dynamic_test_mode,
            change_step=change_step,
            target_friction_scale=target_friction_scale
        )
        if log_dir:
...
def train_manager(algo_type: str):
    ...
    print(f"\n>>> Eğitim Başlıyor: {algo_type.upper()} (Friction Randomization)")
    ...
    if algo_type == 'lstm':
        model = RecurrentPPO(
            ...
            batch_size=512, policy_kwargs={"lstm_hidden_size": 256, "shared_lstm": True, "enable_critic_lstm": False}
        )
    else:
...
# %% [6] VISUALIZATION (A/B TEST - FRICTION)
def plot_friction_results():
    print("\n>>> Sürtünme Analizi (A/B Test) Başlıyor...")
    
    plt.rcParams.update({
        "font.family": "serif", "font.size": 11, "lines.linewidth": 2.0, "figure.autolayout": True
    })
    ...
    scenarios = [
        {"name": f"A) Sudden Friction Increase ({CFG.friction_multiplier}x)", "mult": CFG.friction_multiplier, "ax": axes[0]},
        {"name": "B) Control (Constant Friction)", "mult": 1.0, "ax": axes[1]}
    ]

    for scen in scenarios:
        friction_mult = scen["mult"]
        ax = scen["ax"]
        print(f"\n--- Senaryo: {scen['name']} ---")
        ...
        for name, conf in models_config.items():
            ...
            for ep in range(30):
                ...
                # 100 Adım (50 normal + 50 sürtünmeli)
                for t in range(100):
        ...
        if list_adaptation:
            ...
            ax.axvline(x=CFG.change_step, color='k', linestyle='-', alpha=0.4)
            ax.set_title(scen["name"], fontweight="bold")
            ax.grid(True, ls="--", alpha=0.3)
            
            if friction_mult > 1.0:
                ax.text(CFG.change_step+2, ax.get_ylim()[1]*0.9, "FRICTION SPIKE!", color='red', fontweight='bold')

    plt.tight_layout()
    save_path = os.path.join(CFG.result_dir, "Friction_Analysis.png")
...
if __name__ == "__main__":
    # ÖNCE EĞİTİM YAPMALISIN! (Yeni klasöre kaydedecek)
    TRAIN_MODE = False 
